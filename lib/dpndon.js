 ...
 */
module.exports = ({pkgName, dependencyTypes, props}) => {
  const loader = createPropLoader(pkgName)
  const dependenciesArray = dependencyTypes.map(type => {
    return loader.loadProp(type)
    .then(dependencies => {
      return Object.entries(dependencies || {}).map(([name, version]) => {
        const loader = createPropLoader(name, version);
 ...
 */
function createPropLoader(pkgName, version) {
  return new PropertyLoader(new PackageJsonLoader(pkgName, version));
}